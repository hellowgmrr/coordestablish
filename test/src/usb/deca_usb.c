...
uint8* version;
int s1configswitch;
extern uint32_t pauseTWRReports ;
extern uint8_t sendTWRRawReports ;
		...
		//			setLCDline1(switchS1);
				...
				if(local_buff[0] == 100) //d (from "deca")
				{
					...
					if(local_buff[4] == 36) //"$"
					{
						...
						pauseTWRReports = portGetTickCount();
						result = 2;
						if(local_buff[3] == 'A')//outputting to QT GUI - don't send "mr" reports
						{
							sendTWRRawReports = 0;
						}
...
//		unsigned char tcp_data1[] = "tcp 客户端实验!\n";
	    {
	...
	//       	   if(1)
	        	{
	        		led_on(LED_PC8);
	        		TCP_Client_Send_Data(pcb,tx_buff, tx_buff_length);
//	        		TCP_Client_Send_Data(pcb,tcp_data1,sizeof(tcp_data1));
	        	}
//	        		Delay_s(0xfffff);
